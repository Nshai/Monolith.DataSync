...
namespace IntelliFlo.Platform.Services.Workflow.v1.Activities
{
    public class WorkflowServiceFactory : IWorkflowServiceFactory, IMapActivity
    {
        ...
        public Activity Map(Domain.CreateTaskStep step, Template template, int stepIndex)
        {
            ...
            var activity = new v1.Activities.CreateTaskStep
            {
                StepId = step.Id,
                StepIndex = stepIndex,
        ...
        {
            var activity = new v1.Activities.DelayStep
            {
                StepId = step.Id,
                StepIndex = stepIndex,
        ...
        private int GetUserPartyId(int userId)
        {
            using (var crmClient = clientFactory.Create("crm"))
            {
                ...
                Check.IsTrue(claims.ContainsKey(Principal.Constants.ApplicationClaimTypes.PartyId), "Couldn't retrieve party id claim for user id {0}", userId);
