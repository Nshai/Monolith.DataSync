...
namespace IntelliFlo.Platform.Services.Workflow.Tests
{
    ...
    public class TemplateResourceTests
    {
        ...
        public void SetUp()
        {
            ...
            identity.AddClaim(new Claim(Principal.Constants.ApplicationClaimTypes.UserId, OwnerUserId.ToString(CultureInfo.InvariantCulture)));
            identity.AddClaim(new Claim(Principal.Constants.ApplicationClaimTypes.TenantId, TenantId.ToString(CultureInfo.InvariantCulture)));
            identity.AddClaim(new Claim(Principal.Constants.ApplicationClaimTypes.Subject, Guid.NewGuid().ToString()));
            identity.AddClaim(new Claim(Principal.Constants.ApplicationClaimTypes.RoleId, RoleId.ToString(CultureInfo.InvariantCulture)));
            identity.AddClaim(new Claim(Principal.Constants.ApplicationClaimTypes.GroupLineage, string.Join(",", new[] { GroupId, ParentGroupId })));
